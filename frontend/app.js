(function () {
  ...
  // CRITICAL LINK: Connected directly to your live production backend Vercel service
  const API_URL = 'https://backend-woad-kappa-55.vercel.app';
  ...
  function bindEvents() {
    ...
    if (bookingForm) {
      bookingForm.addEventListener('submit', function (e) {
        ...
        // Fire network submission sequence directly into the correct API route path
        ...
        .then(function (res) {
          if (!res.ok) throw new Error('Booking failed. Please check your network or database connectivity.');
          return res.json();
        })
        .then(function (data) {
          // Success alert displaying the real-time generated tracking token!
          alert('Success! Your XYZ Dental Clinic appointment token is: ' + data.token);
          bookingForm.reset();
        })
        .catch(function (err) {
